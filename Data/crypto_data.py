...
import datetime
import time
from urllib.error import HTTPError #error handling with connection to CoinGecko server
import sys
cg = CoinGeckoAPI()
...
def crypto_data_collector(user_start_time, user_end_time): # Make function for collecting data from Coin Gecko, so that module doesn't run when imported
    ...
    *****************************************************************************************************************************************************
    Returns: 2 .csv files into "Data/csv_files/" directory (overwrites/updates existing .csv of same name) and a dataframe
    ...
    # Error handling for HTTPError from built-in urllib library
    try:
        ...
        print('Server Not avaialable try again later for most up-to-date data. Data from load will be used if available')
    ...
    coin_list_df
    coin_list_df.to_csv('Data/csv_files/coin_list.csv')
    ...
    coin_list_df = pd.read_csv('Data/csv_files/coin_list.csv')
    
    # Grabbing coin id's and using them to get price history data from CoinGecko via API calls
    # Error handling for HTTPError from built-in urllib library
    
    prices_dictionary = {} #empty dictionary for capturing coin's data from the API Calls
    ...
    for CG_id in coin_list_df['CG id']:
        try: 
            ...
            time.sleep(1) #adds a 1 second delay every call staying with in CoinGecko 100 calls/min (60 secs/100 calls) limit as stated here--> https://realpython.com/python-sleep/
        except:
            print('Server Not avaialable try again later for most up-to-date data. Data from previous sessions will be used if available')
            crypto_price_df = pd.read_csv('Data/csv_files/crypto_price_data.csv')
    # Concatenating all dataframes together along columns
    coin_historical_df = pd.concat(df_list, axis=1, join='outer') 
    coin_historical_df.to_csv('Data/csv_files/crypto_price_data.csv')
    crypto_price_df = coin_historical_df
